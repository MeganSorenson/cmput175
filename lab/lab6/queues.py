# Bounded and Circular Queues
# implements the bounded queue and circular queue ADTs using lists
# Author: CMPUT 175
# Modified by: Megan Sorenson

class BoundedQueue:
    # Creates a new empty queue:
    # head is index zero and tail is end of list
    def __init__(self, capacity):
        assert isinstance(capacity, int), ('Error: Type error: %s' % (
            type(capacity)))  # throws an assertion error on not true
...
class CircularQueue:
    ...
    def __init__(self, capacity):
        ...
        assert isinstance(
            capacity, int), ('Error: Type error: %s' % (type(capacity)))
        assert capacity >= 0, ('Error: Illegal capacity: %d' % (capacity))
